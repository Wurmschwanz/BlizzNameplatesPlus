...
local function ApplyCastbarFont(text)
  ...
  if currentSize ~= size then
    -- Keep Blizzard's font face and flags; this option owns only the size.
...
local function ApplyCastbarLayout(plate, bar)
  ...
  bar:ClearAllPoints()
  bar:SetPoint("TOPLEFT", plate.healthbar, "BOTTOMLEFT", 15 + xOffset, -gap + yOffset)
  bar:SetPoint("TOPRIGHT", plate.healthbar, "BOTTOMRIGHT", 14 + xOffset, -gap + yOffset)

  if bar.icon then
    bar.icon:ClearAllPoints()
    bar.icon:SetPoint("TOPLEFT", plate.healthbar, "BOTTOMLEFT", xOffset, -gap + yOffset)
  end
...
local function CreateCastbar(plate)
  ...
  icon:SetWidth(14)
  icon:SetHeight(14)
  icon:SetPoint("TOPLEFT", plate.healthbar, "BOTTOMLEFT", xOffset, -gap + yOffset)
...
function BNP:RefreshCastbarLayout()
  ...
  for plate in pairs(BNP.plates or {}) do
    if plate and plate.BNPCastbar then
      ...
      ApplyCastbarFont(plate.BNPCastbar.text)
