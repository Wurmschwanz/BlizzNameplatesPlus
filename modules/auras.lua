...
local function GetDebuffYOffset()
  if BNP.GetDebuffYOffset then return BNP:GetDebuffYOffset() end
  return 0
end
local function GetCCYOffset()
  if BNP.GetCCYOffset then return BNP:GetCCYOffset() end
  return 0
end
...
local function SharesTopRowFamily()
  -- Top Left/Right are horizontal alignments of the same top aura row.
  -- CCs only have a generic Top position, so when the user disables the
  -- separate CC row they must share the debuff container instead of creating
  -- a second centered container that can overlap larger CC icons.
  return IsTopDebuffPosition(GetDebuffPosition()) and GetCCPosition() == "top"
end

local function UseSeparateCCRow()
  -- The separate-row option applies to every top alignment, not only Top Mid.
  if BNP.AreDebuffsEnabled and not BNP:AreDebuffsEnabled() then return false end
  if BNP.AreCrowdControlEnabled and not BNP:AreCrowdControlEnabled() then return false end
  return SharesTopRowFamily()
    and BNP.IsSeparateCCRowEnabled and BNP:IsSeparateCCRowEnabled()
end

local function UseDedicatedCCContainer()
  -- Independent Y offsets require separate containers whenever the user moves
  -- CCs away from the debuff row. At the default 0/0 values the proven shared
  -- row behavior remains unchanged.
  if SharesTopRowFamily() then
    if GetCCYOffset() ~= GetDebuffYOffset() then return true end
    return UseSeparateCCRow()
  end

  -- Other genuinely different positions use the independent CC container.
  return GetCCPosition() ~= GetDebuffPosition() or UseSeparateCCRow()
end

local function GetAuraOffsetY()
  return GetBaseAuraOffsetY() + GetDebuffYOffset()
end
...
local function GetSideCCOffsetY()
  if UseSeparateCCRow() and BNP.AreCrowdControlEnabled and BNP:AreCrowdControlEnabled() then
    return GetIconSize() + ROW_SPACING + GetCCYOffset()
  end
  return GetCCYOffset()
end
...
local function AnchorAuraContainer(plate, container, isCC)
  ...
  -- When BNP hides the Blizzard name, reclaim that empty vertical space.
  -- Only Top rows move; Left/Right are already centered directly on the bar.
  local nameHidden = (BNP.IsPlateNameHidden and BNP:IsPlateNameHidden(plate)) or plate.BNPNameHidden
  local compactShift = (nameHidden and (isCC and position == "top" or (not isCC and IsTopDebuffPosition(position)))) and -NAME_HIDDEN_COMPACT_SHIFT or 0
  local anchor = plate.healthbar or plate

  if position == "left" then
    container:SetPoint("RIGHT", anchor, "LEFT", -SIDE_SPACING, isCC and GetSideCCOffsetY() or GetDebuffYOffset())
  elseif position == "right" then
    container:SetPoint("LEFT", anchor, "RIGHT", GetRightAuraSpacing(plate), isCC and GetSideCCOffsetY() or GetDebuffYOffset())
  elseif not isCC and IsBottomDebuffPosition(position) then
    if position == "bottom_left" then
      container:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -BOTTOM_SPACING + GetDebuffYOffset())
    elseif position == "bottom_right" then
      container:SetPoint("TOPRIGHT", anchor, "BOTTOMRIGHT", 0, -BOTTOM_SPACING + GetDebuffYOffset())
    else
      container:SetPoint("TOP", anchor, "BOTTOM", 0, -BOTTOM_SPACING + GetDebuffYOffset())
    end
  elseif not isCC and position == "top_left" then
    if plate.healthbar then
      container:SetPoint("BOTTOMLEFT", plate.healthbar, "TOPLEFT", 0, GetAuraOffsetY() + compactShift)
    else
      container:SetPoint("BOTTOMLEFT", plate, "BOTTOMLEFT", 0, 24 + GetDebuffYOffset() + compactShift)
    end
  elseif not isCC and position == "top_right" then
    if plate.healthbar then
      container:SetPoint("BOTTOMRIGHT", plate.healthbar, "TOPRIGHT", 0, GetAuraOffsetY() + compactShift)
    else
      container:SetPoint("BOTTOMRIGHT", plate, "BOTTOMRIGHT", 0, 24 + GetDebuffYOffset() + compactShift)
    end
  else
    if plate.healthbar then
      container:SetPoint("BOTTOM", plate.healthbar, "TOP", 0, (isCC and GetCCOffsetY() or GetAuraOffsetY()) + compactShift)
    else
      container:SetPoint("BOTTOM", plate, "BOTTOM", 0, 24 + (isCC and (GetCCOffsetY() - GetBaseAuraOffsetY()) or GetDebuffYOffset()) + compactShift)
    end
  end
end

function BNP:RefreshAuraLayoutForPlate(plate)
  if not plate then return end
  if plate.BNPAuraContainer then AnchorAuraContainer(plate, plate.BNPAuraContainer, false) end
  if plate.BNPCCContainer then AnchorAuraContainer(plate, plate.BNPCCContainer, true) end
  plate.BNPAuraLastNameHidden = (BNP.IsPlateNameHidden and BNP:IsPlateNameHidden(plate)) or (plate.BNPNameHidden and true or false)
end
...
local function PositionAuraIcon(icon, container, index, visibleCount, iconSize, isCC)
  ...
  if position == "left" or position == "bottom_right" or position == "top_right" then
    icon:SetPoint("RIGHT", container, "RIGHT", -((index - 1) * step), 0)
  elseif position == "right" or position == "bottom_left" or position == "top_left" then
...
-- Full-immunity cleanse bridge ----------------------------------------------
-- Divine Shield and Ice Block remove harmful effects. BNP tracks some of
-- those effects from cast events/timers, so a server cleanse must also wipe
-- every local harmful-aura cache for that exact GUID. While the immunity is
-- active, keep a short renewable suppression window so stale UnitDebuff /
-- pending-cast data cannot repopulate icons for one frame.
BNP.guidHarmfulAuraSuppressionUntil = BNP.guidHarmfulAuraSuppressionUntil or {}

local function ClearHarmfulAuraStateOnly(guid)
  if not guid then return end
  BNP.guidAuras[guid] = nil
  if BNP.svStateByGUID then BNP.svStateByGUID[guid] = nil end
  if BNP.svExpiryByGUID then BNP.svExpiryByGUID[guid] = nil end
  ...
  -- A failed/immune cast rollback must never restore pre-cleanse timers after
  -- Bubble/Ice Block. Drop all outstanding attempts tied to this GUID.
  if BNP.auraCastAttempts then
    local i
    for i = table.getn(BNP.auraCastAttempts), 1, -1 do
      local attempt = BNP.auraCastAttempts[i]
      if attempt and attempt.guid == guid then
...
local function HarmfulAuraSuppressed(guid, now)
  if not guid then return false end
  local untilTime = BNP.guidHarmfulAuraSuppressionUntil[guid]
  if not untilTime then return false end
  now = now or GetTime()
  if now < untilTime then return true end
  BNP.guidHarmfulAuraSuppressionUntil[guid] = nil
  return false
end

function BNP:SuppressHarmfulAurasForImmunity(guid, seconds)
  if not guid then return end
  local now = GetTime()
  local untilTime = now + math.max(tonumber(seconds) or 0.35, 0.10)
  local current = self.guidHarmfulAuraSuppressionUntil[guid] or 0
  if untilTime > current then
    self.guidHarmfulAuraSuppressionUntil[guid] = untilTime
  end
  ClearHarmfulAuraStateOnly(guid)
end

function BNP:IsHarmfulAuraSuppressed(guid, now)
  return HarmfulAuraSuppressed(guid, now)
end
...
local function FailPendingAura(targetGUID, spellID)
  ...
  if def then
    if entries[def.key] then entries[def.key] = nil end
    if def.secondaryAuraKey and entries[def.secondaryAuraKey] then
      entries[def.secondaryAuraKey] = nil
    end
  end
...
local STRICT_AURA_KEYS = {
  ...
  serpent_sting=true, viper_sting=true, scorpid_sting=true,
...
local function BuildAffectedKeys(def)
  ...
  keys[def.key] = true
  if def.secondaryAuraKey then keys[def.secondaryAuraKey] = true end
...
local function RollbackAuraCastAttempt(attempt, reason)
  ...
  -- A failed refresh must not leave a pending confirmation behind. Otherwise
  -- the still-active old aura could be mistaken for the newly failed cast on
  -- the next UnitDebuff scan and incorrectly restart its timer.
  local pending = BNP.pendingAuras and BNP.pendingAuras[guid]
  if pending then
    for key in pairs(attempt.before or {}) do pending[key] = nil end
  end
...
-- A positive UnitDebuff sighting proves a first application landed. Disarm
-- that cast attempt immediately so a later, unrelated failed button press for
-- the same spell cannot restore the pre-cast (empty) state. Refresh attempts
-- intentionally stay armed: an already-present aura cannot prove that the
-- refresh succeeded, and a delayed miss/resist must still restore its timer.
-- Keep this on BNP instead of adding another chunk-local function; the Vanilla
-- client has a strict local-variable limit for large Lua files.
function BNP:ResolveNewAuraCastAttempt(guid, key)
  ...
  for i = table.getn(BNP.auraCastAttempts), 1, -1 do
    local attempt = BNP.auraCastAttempts[i]
    local prior = attempt and attempt.before and attempt.before[key]
    if attempt and not attempt.done
      and attempt.guid == guid
      and now - (attempt.time or 0) <= AURA_ATTEMPT_WINDOW
      and prior and not prior.exists then
      attempt.done = true
      return true
    end
  end
...
-- Do not match SPELLFAILCASTSELF here. That message describes a newly rejected
-- input (for example pressing Corruption again during the global cooldown),
-- not the result of the earlier UNIT_CASTEVENT CAST. Matching it by spell name
-- could therefore roll back a DoT that already landed. Exact SuperWoW FAIL and
-- the impact-result patterns above still handle real miss/resist/immune cases.
...
castEvents:SetScript("OnEvent", function()
  ...
  -- Turtle WoW scales Rip from 10 to 18 seconds with one to five combo
  -- points. Resolve this after tooltip/rank detection so the cast's actual
  -- combo-point count remains authoritative.
  if def.comboPointDurations and BNP.GetTrackedComboPointsForGUID then
    local comboPoints = BNP:GetTrackedComboPointsForGUID(targetGUID)
    local comboDuration = def.comboPointDurations[comboPoints]
    if comboDuration then duration = comboDuration end
  end
  ...
  -- Pounce triggers a separate 18-second bleed spell. Queue its real spell ID
  -- and let the normal UnitDebuff confirmation path prove that it landed on
  -- this exact GUID before any icon or timer is shown. Keep this inline: the
  -- 1.12 Lua client has a strict local-variable limit for the main chunk.
  if def.secondaryAuraKey and def.secondarySpellIDs then
    local numericSpellID = tonumber(spellID) or spellID
    local secondarySpellID = def.secondarySpellIDs[numericSpellID]
    local secondaryDef = nil

    if secondarySpellID then
      local _, candidate
      for _, candidate in ipairs(AURA_DEFS) do
        if candidate.key == def.secondaryAuraKey then
          secondaryDef = candidate
          break
        end
      end
    end

    if secondaryDef then
      local secondaryTexture = texture or secondaryDef.texture
      if SpellInfo then
        local _, _, foundTexture = SpellInfo(secondarySpellID)
        secondaryTexture = foundTexture or secondaryTexture
      end

      pendingForGUID[secondaryDef.key] = {
        def = secondaryDef,
        spellID = secondarySpellID,
        texture = secondaryTexture,
        duration = secondaryDef.duration or 18,
        created = GetTime(),
      }
      RaidTrace("SECONDARY_PENDING", targetGUID, secondaryDef.key, secondarySpellID)
    end
  end
...
local function LayoutActiveIcons(container, active, visibleCount, forceCC)
  ...
  elseif position == "right" or position == "bottom_left" or position == "top_left" then
    ...
    if position == "top" then
      -- Match the visual center used by the health text.
      cursor = cursor + 3
    end
    for i = 1, visibleCount do
      local icon = container.icons[i]
...
local function SyncForeignCCs(unit, guid, now)
  ...
  foreignCCSeenGeneration = foreignCCSeenGeneration + 1
  ...
  if live then
    ...
    for key, aura in pairs(live) do
      if FOREIGN_CC_SEEN[key] ~= seenGeneration then
...
local function SyncAuraRemoval(unit, guid, now, snapshot, currentTargetGUID)
  ...
  removalPresentGeneration = removalPresentGeneration + 1
  local presentGeneration = removalPresentGeneration
  local i
  local maxIndex = snapshot and snapshot.count or 64
  for i = 1, maxIndex do
  ...
  currentTargetGUID = currentTargetGUID or GetTargetGUID()
...
local function PinExactTargetPlate(plate, now)
  ...
  -- RefreshTargetPriorityIdentity() already resolves the exact ClassicAPI target
  -- plate once at the start of every renderer tick. Reuse that result here. The
  -- old code repeated the same protected GetNamePlateForUnit("target") call for
  -- every visible plate at 20 Hz, which was pure duplicate work in large raids.
  local targetGUID = targetPriorityGUID
  local targetPlate = targetPriorityPlate
  if not targetGUID or not targetPlate or targetPlate ~= plate then return nil end
...
-- Shared identity helper for sibling nameplate modules (for example PvP
-- immunities). Reusing the same stabilized GUID prevents those modules from
-- reintroducing the dense-raid/nameplate-token flicker fixed above.
BNP.GetStablePlateAuraGUID = GetStablePlateGUID
...
local function UpdatePlate(plate, now)
  ...
  -- Nameplate frames are recycled. Re-anchor only when the hidden-name state
  -- changes so Player/NPC hide options immediately compact Top aura rows.
  local nameHidden = (BNP.IsPlateNameHidden and BNP:IsPlateNameHidden(plate)) or (plate.BNPNameHidden and true or false)
  if plate.BNPAuraLastNameHidden ~= nameHidden then
    plate.BNPAuraLastNameHidden = nameHidden
  ...
  -- A full-immunity cleanse (Divine Shield / Ice Block) is authoritative.
  -- Hide both harmful rows immediately and do not let timer/cast caches render
  -- again until the short suppression window is no longer being renewed by
  -- the immunity module.
  if guid and HarmfulAuraSuppressed(guid, now) then
    ClearHarmfulAuraStateOnly(guid)
    HidePlateAuraFrames(plate)
    return
  end
...
local removalScanBucket = 0
local nextRemovalBucket = 0

local function GetRemovalScanBucket(plate)
  if plate.BNPAuraRemovalScanBucket == nil then
    plate.BNPAuraRemovalScanBucket = nextRemovalBucket
    nextRemovalBucket = 1 - nextRemovalBucket
  end
  return plate.BNPAuraRemovalScanBucket
end
renderer:SetScript("OnUpdate", function()
  ...
  -- every individual plate keeps the same ~0.10s cadence, but CPU/UnitDebuff
  -- work is spread across two frames to reduce micro-stutter.
  removalScanBucket = 1 - removalScanBucket

  local targetGUID = GetTargetGUID()

  local targetSuppressed = targetGUID and HarmfulAuraSuppressed(targetGUID, now)
  if targetSuppressed then
    ClearHarmfulAuraStateOnly(targetGUID)
  end

  if hasPending and targetGUID and not targetSuppressed then
    -- Current target is the most reliable confirmation source.
    ConfirmPendingForUnit("target", targetGUID, now)
  end

  if targetGUID and not targetSuppressed then
  ...
  for plate in pairs(BNP.plates) do
    ...
    if plate:IsShown() and (hasPending or hasPendingAoE or plateRemovalDue) then
      ...
      if guid and token then
        local guidSuppressed = HarmfulAuraSuppressed(guid, now)
        if guidSuppressed then
          ClearHarmfulAuraStateOnly(guid)
        end
        ...
        if hasPending and not isCurrentTarget and not guidSuppressed then
          ConfirmPendingForUnit(token, guid, now)
        end

        if hasPendingAoE and not guidSuppressed then
          ConfirmAoEAurasOnUnit(token, guid)
        end

        if plateRemovalDue and not isCurrentTarget and not guidSuppressed then
          ...
          if identityTrusted then
            ...
            SyncAuraRemoval(token, stableGUID, now, snapshot, targetGUID)
          end
        end
      end
    end
...
local FOREIGN_CC_BUCKET_INTERVAL = FOREIGN_CC_SCAN_INTERVAL / 2
local foreignCCElapsed = 0
local foreignCCTargetElapsed = 0
local foreignCCScanBucket = 0
local foreignCCNextBucket = 0

local function GetForeignCCScanBucket(plate)
  if plate.BNPForeignCCScanBucket == nil then
    plate.BNPForeignCCScanBucket = foreignCCNextBucket
    foreignCCNextBucket = 1 - foreignCCNextBucket
  end
  return plate.BNPForeignCCScanBucket
end
...
foreignCCScanner:SetScript("OnUpdate", function()
  ...
  foreignCCElapsed = 0
  foreignCCScanBucket = 1 - foreignCCScanBucket
