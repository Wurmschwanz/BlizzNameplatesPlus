...
local function UseSeparateCCRow()
  -- A separate CC row only makes sense for the Top layout. Left/Right already
  -- grow horizontally away from the nameplate, so keep CCs in the same row.
  if BNP.GetDebuffPosition and BNP:GetDebuffPosition() ~= "top" then
    return false
  end
...
-- Aura frames stay inside their native projected nameplate hierarchy. This
-- lets the permanent target FrameLevel raise the complete target plate above
-- every non-target aura without moving nameplates into Blizzard UI strata.
...
local function PositionAuraIcon(icon, container, index, visibleCount, iconSize)
...
local function ApplyTargetAuraPriority(plate, container)
  ...
  if not targetPriorityGUID then
    SetTargetPriorityAuraAlpha(container, 1)
    return
  end

  local isTarget = plate.BNPIsCurrentTarget or
                   (targetPriorityPlate and plate == targetPriorityPlate) or
                   (plate.BNPAuraGUIDStable and plate.BNPAuraGUIDStable == targetPriorityGUID)

  -- Keep exact unit/GUID fallbacks for the first renderer tick after a newly
  -- projected nameplate appears, before fullalpha has published its marker.
  if not isTarget and targetPriorityGUID and plate.GetName then
    local token = plate:GetName(1)
    if token == targetPriorityGUID then
      isTarget = true
    elseif token then
      if UnitIsUnit then
        local ok, sameUnit = pcall(UnitIsUnit, token, "target")
        if ok and sameUnit then isTarget = true end
      end
      if not isTarget then
        local exists, guid = UnitExists(token)
        if exists and guid == targetPriorityGUID then isTarget = true end
      end
    end
  end

  SetTargetPriorityAuraAlpha(
    container,
    isTarget and 1 or math.min(
      1,
      TARGET_PRIORITY_AURA_ALPHA /
        math.max(0.01, (plate.GetAlpha and plate:GetAlpha()) or 1)
    )
  )
end
...
local AURA_GUID_TRANSIENT_HOLD = 0.20
...
local function GetStablePlateGUID(plate, now)
  ...
  -- A projected token can vanish for one renderer tick while many nameplates
  -- overlap. Previously that immediately cleared the stable GUID and hid every
  -- icon, producing the raid-only DoT flicker. Keep the last confirmed identity
  -- for a tiny bounded window instead.
  if not guid then
  ...
  if stable and guid == stable then
    ...
    plate.BNPAuraGUIDCandidateCount = AURA_GUID_CONFIRM_UPDATES
  ...
  if plate.BNPAuraGUIDCandidate ~= guid then
    ...
    -- While a different GUID is being verified, keep rendering the previous
    -- stable GUID only if this plate never went through an OnHide/OnShow reuse.
    -- This removes one-frame overlap flicker without reviving stale auras on a
    -- genuinely recycled Blizzard nameplate.
    if stable and not plate.BNPAuraForceFreshIdentity then
      return stable
    end
    return nil
  end
  ...
  if plate.BNPAuraGUIDCandidateCount < AURA_GUID_CONFIRM_UPDATES then
    if stable and not plate.BNPAuraForceFreshIdentity then return stable end
    return nil
  end
