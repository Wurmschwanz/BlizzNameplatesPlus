...
local function UseDedicatedCCContainer()
  -- Top Mid/Left/Right are the same logical row when Separate CC Row is off.
  -- LayoutActiveIcons() already accounts for mixed debuff/CC icon sizes, so
  -- keeping them in one container prevents overlap (e.g. 18px debuffs + 24px CCs).
...
local function AnchorAuraContainer(plate, container, isCC)
  ...
  if position == "left" then
    container:SetPoint("RIGHT", anchor, "LEFT", -SIDE_SPACING, isCC and GetSideCCOffsetY() or 0)
  elseif position == "right" then
    container:SetPoint("LEFT", anchor, "RIGHT", GetRightAuraSpacing(plate), isCC and GetSideCCOffsetY() or 0)
  elseif not isCC and IsBottomDebuffPosition(position) then
    if position == "bottom_left" then
      container:SetPoint("TOPLEFT", anchor, "BOTTOMLEFT", 0, -BOTTOM_SPACING)
    elseif position == "bottom_right" then
      container:SetPoint("TOPRIGHT", anchor, "BOTTOMRIGHT", 0, -BOTTOM_SPACING)
    else
      container:SetPoint("TOP", anchor, "BOTTOM", 0, -BOTTOM_SPACING)
    end
  elseif not isCC and position == "top_left" then
    if plate.healthbar then
      ...
      container:SetPoint("BOTTOMLEFT", plate, "BOTTOMLEFT", 0, 24 + compactShift)
    end
  elseif not isCC and position == "top_right" then
    if plate.healthbar then
      ...
      container:SetPoint("BOTTOMRIGHT", plate, "BOTTOMRIGHT", 0, 24 + compactShift)
    end
  else
    if plate.healthbar then
      container:SetPoint("BOTTOM", plate.healthbar, "TOP", 0, (isCC and GetCCOffsetY() or GetAuraOffsetY()) + compactShift)
    else
      container:SetPoint("BOTTOM", plate, "BOTTOM", 0, 24 + (isCC and (GetCCOffsetY() - GetAuraOffsetY()) or 0) + compactShift)
    end
  end
end
