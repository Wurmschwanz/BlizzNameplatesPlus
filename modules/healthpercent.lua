...
local function ApplyHealthTextFont(text)
  if not text then return end

  local size = BNP.GetHealthTextFontSize and BNP:GetHealthTextFontSize() or 9
  local outline = BNP.GetHealthTextOutline and BNP:GetHealthTextOutline() or "outline"
  local flags = ""
  ...
  -- Avoid reapplying the font on every health update. Only touch the FontString
  -- when the user actually changes size/outline or when the text is first made.
  if text.BNPFontSize ~= size or text.BNPFontOutline ~= outline then
    text:SetFont("Fonts\\FRIZQT__.TTF", size, flags)
    text.BNPFontSize = size
    text.BNPFontOutline = outline
  end
end
...
local elapsed = 0
local healthScanBucket = 0
local healthNextBucket = 0

local function GetHealthScanBucket(plate)
  if plate.BNPHealthScanBucket == nil then
    plate.BNPHealthScanBucket = healthNextBucket
    healthNextBucket = 1 - healthNextBucket
  end
  return plate.BNPHealthScanBucket
end
...
updater:SetScript("OnUpdate", function()
  ...
  -- Health text is off by default. Do not walk every known nameplate when there
  -- is literally nothing to update. Option changes refresh immediately.
  if not Enabled() then return end

  -- Keep the old 0.10s per-plate update rate, but distribute it over two frames.
  healthScanBucket = 1 - healthScanBucket

  local plate
  for plate in pairs(BNP.plates or {}) do
    if GetHealthScanBucket(plate) == healthScanBucket then
