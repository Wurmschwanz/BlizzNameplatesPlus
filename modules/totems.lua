...
local function FindTotemDefinition(name)
  ...
  -- complete word though: player names such as "Totemtiger" must never be
  -- mistaken for a totem just because they contain those five letters.
  if normalized == "totem"
    or string.sub(normalized, 1, 6) == "totem "
    or string.sub(normalized, -6) == " totem"
    or string.find(normalized, " totem ", 1, true) then
...
local TOTEM_VISUAL_RESCAN_INTERVAL = 0.50

local function CaptureTotemPlateVisuals(plate, state, now)
  if not plate or not state then return end

  -- Capturing every projected region/child allocates temporary Lua tables.
  -- Doing that every rendered frame produced needless GC pressure in BGs with
  -- several totems. Re-scan only occasionally so late-created BNP children
  -- (castbar/combo/etc.) are still discovered without per-frame allocations.
  ...
  state.nextCapture = (now or GetTime()) + TOTEM_VISUAL_RESCAN_INTERVAL
end

local function SuppressTotemPlateVisuals(plate, forceCapture)
  ...
  local now = GetTime()
  if not state then
    state = { alpha = {}, indicator = indicator, nextCapture = 0 }
  ...
  -- Blizzard can restore alpha on its native regions every frame. Re-enforce
  -- only the already-cached objects here; this loop allocates nothing.
  local object
  for object in pairs(state.alpha or {}) do
    if object ~= indicator and object.GetAlpha and object.SetAlpha and object:GetAlpha() ~= 0 then
      object:SetAlpha(0)
    end
  end

  -- BNPTotemIndicator is itself a direct child of the plate.
  if indicator and indicator.SetAlpha and indicator.GetAlpha and indicator:GetAlpha() ~= 1 then
  ...
  plate:SetScript("OnUpdate", function()
    ...
    if current.BNPTotemLastKey and
      ...
      if current.BNPTotemIndicator then
        if current.BNPTotemIndicator.GetAlpha and current.BNPTotemIndicator:GetAlpha() ~= 1 then
          current.BNPTotemIndicator:SetAlpha(1)
        end
        if not current.BNPTotemIndicator:IsShown() then current.BNPTotemIndicator:Show() end
      end
...
local function UpdateIndicator(plate, force)
  ...
  indicator:Show()
  SuppressTotemPlateVisuals(plate, force)
end
