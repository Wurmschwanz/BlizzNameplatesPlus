...
local function GetImmunityYOffset()
  if BNP.GetImmunityYOffset then return BNP:GetImmunityYOffset() end
  return 0
end
...
local function AnchorContainer(plate, container)
  ...
  if position == "left" then
    container:SetPoint("RIGHT", anchor, "LEFT", -SIDE_SPACING, GetSideOffset("left") + GetImmunityYOffset())
  elseif position == "right" then
    container:SetPoint("LEFT", anchor, "RIGHT", GetRightSpacing(plate), GetSideOffset("right") + GetImmunityYOffset())
  else
    ...
    if plate.healthbar then
      container:SetPoint("BOTTOM", plate.healthbar, "TOP", 0, GetTopOffset() + GetImmunityYOffset() + compactShift)
    else
      container:SetPoint("BOTTOM", plate, "BOTTOM", 0, 24 + GetTopOffset() + GetImmunityYOffset() + compactShift)
    end
  end
end
...
local function CreateIcon(parent, index)
  ...
  local timerSize = math.floor(((UI.TIMER_SIZE or 8) * size / (UI.ICON_SIZE or 18)) + 0.5)
...
local function ResetImmunitySeen()
  local key
  for key in pairs(IMMUNITY_SEEN) do IMMUNITY_SEEN[key] = nil end
  return IMMUNITY_SEEN
end
...
local function Render(plate, guid, now)
  ...
  -- Reuse a tiny render list/pool per container. This path runs frequently in
  -- BGs, so avoid allocating a new active table and item tables every scan.
  local active = container.BNPActiveImmunities
  if not active then active = {}; container.BNPActiveImmunities = active end
  local pool = container.BNPActiveImmunityPool
  if not pool then
    pool = {}
    local poolIndex
    for poolIndex = 1, MAX_ICONS do pool[poolIndex] = {} end
    container.BNPActiveImmunityPool = pool
  end
  local oldCount = table.getn(active)
  local clearIndex
  for clearIndex = 1, oldCount do active[clearIndex] = nil end
  ...
  for d = 1, table.getn(IMMUNITY_DEFS) do
    ...
    if entry and (not entry.expires or (remaining and remaining > 0)) then
      ...
      if activeCount < MAX_ICONS then
        activeCount = activeCount + 1
        local item = pool[activeCount]
        item.def = def
        item.entry = entry
...
-- frame. Split that work into two 0.05s buckets while preserving the same
-- ~0.10s per-nameplate scan cadence.
local immunityElapsed = 0
local immunityScanBucket = 0
local immunityNextBucket = 0

local function GetImmunityScanBucket(plate)
  if plate.BNPImmunityScanBucket == nil then
    plate.BNPImmunityScanBucket = immunityNextBucket
    immunityNextBucket = 1 - immunityNextBucket
  end
  return plate.BNPImmunityScanBucket
end

local immunityScanner = CreateFrame("Frame")
immunityScanner:SetScript("OnUpdate", function()
  immunityElapsed = immunityElapsed + arg1
  if immunityElapsed < 0.05 then return end
  immunityElapsed = 0

  if BNP.ArePvPImmunitiesEnabled and not BNP:ArePvPImmunitiesEnabled() then return end

  immunityScanBucket = 1 - immunityScanBucket
  local plate
  for plate in pairs(BNP.plates or {}) do
    if plate and plate:IsShown() and GetImmunityScanBucket(plate) == immunityScanBucket then
      SafeUpdatePlate(plate)
    end
  end
end)
