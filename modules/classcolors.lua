...
local function GetClassScanBucket(plate)
  if plate.BNPClassScanBucket == nil then
    plate.BNPClassScanBucket = nextBucket
    nextBucket = 1 - nextBucket
  end
  return plate.BNPClassScanBucket
end
...
updater:SetScript("OnUpdate", function()
  ...
  -- Split the old 0.10s all-plate burst into two 0.05s buckets. Every plate is
  -- still checked once per 0.10s, but large BG/raid updates no longer land in
  -- one Lua spike.
  scanBucket = 1 - scanBucket

  local plate
  for plate in pairs(BNP.plates or {}) do
    if plate:IsShown() and plate.healthbar and GetClassScanBucket(plate) == scanBucket then
      if plate.BNPClassGUID then
        -- EnforceClassColor already validates token/GUID/player identity. The old
        -- updater resolved the same unit once here and a second time inside it.
