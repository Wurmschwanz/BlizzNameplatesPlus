...
local function ReapplyShaguDarkMode(plate)
  ...
  -- Dark Nameplate Border is an explicit BNP option and therefore owns this
  -- one texture. Re-apply its selected state after Shagu's immediate and
  -- delayed passes, otherwise an old dark tint can remain stuck after the
  -- checkbox is switched off.
  if BNP.RepairNativeNameplateVisuals then
    BNP:RepairNativeNameplateVisuals(plate)
  end
end
