...
local function StateYOffset(state)
  if state and state.BNPNameAnchor and BNP.GetNameFontYOffset then
    return BNP:GetNameFontYOffset()
  end
  return 0
end
...
local function AnchorStateMatches(state)
  ...
  for i = 1, expectedCount do
    ...
    if point ~= expected.point
      ...
      or math.abs((y or 0) - (expected.y + StateYOffset(state))) > 0.01 then
      return false
    end
  end
...
-- SuperWoW exposes the attached unit GUID through nameplate:GetName(1).
-- Its SetMouseoverUnit(unit) API is specifically intended for custom unit
-- frames: set it on OnEnter and clear it on OnLeave.  BNP's scale/Y-offset
...
local function SetWrapperMouseover(plate, wrapper)
  local unit = GetPlateMouseoverUnit(plate)
  if not unit then return end

  -- Keep ownership so a late OnLeave from an overlapping/recycled plate cannot
  -- clear the mouseover unit that a newer wrapper has just established.
  BNP.BNPMouseoverOwner = wrapper
  wrapper.BNPMouseoverUnit = unit

  if SetMouseoverUnit then
    local ok = pcall(SetMouseoverUnit, unit)
    if ok then
      ShowPlateTooltip(wrapper, unit)
    end
  end
end

local function ClearWrapperMouseover(wrapper)
  if BNP.BNPMouseoverOwner ~= wrapper then return end
...
local function SetYOffsetMouseMode(plate, wrapper, offset)
  ...
  if not wrapper.BNPClickForwardInstalled then
    wrapper:SetScript("OnClick", function()
      -- The wrapper exists only because BNP visually offsets/scales the original
      -- Blizzard plate. Keep Blizzard's native nameplate click semantics by
      -- forwarding the ACTUAL mouse button to the original Button.
      --
      -- parent:Click() without an argument always simulates LeftButton. That was
      -- the reason right click turned into a plain target-select while BNP was
      -- enabled. Do not call AttackTarget() manually here: it toggles auto attack
      -- and is not equivalent to Blizzard's native nameplate right-click path.
  ...
  if not wrapper.BNPMouseoverFeedbackInstalled then
    wrapper:SetScript("OnEnter", function()
      ...
      SetWrapperMouseover(parent, current)
    end)
    ...
    wrapper:SetScript("OnLeave", function()
      ...
      ClearWrapperMouseover(current)
    end)

    -- A projected nameplate can disappear/recycle while the cursor is still
    -- inside its old bounds. Clear SuperWoW's synthetic mouseover in that case
    -- as well so "mouseover" can never remain stuck on a vanished unit.
...
local function EnsureScaleWrapper(plate)
  ...
  for i, object in pairs(regions) do
    if object and object.SetParent then
      ...
      if state then
        if object == plate.name then
          state.BNPNameAnchor = true
        end
        table.insert(wrapper.BNPAnchorStates, state)
      end
...
function BNP:ApplyNameFontYOffset(plate)
  ...
  for i = 1, table.getn(states) do
    ...
    if state and state.BNPNameAnchor then
      RestoreAnchorState(state, wrapper)
    end
  end
end

function BNP:ApplyNameFontYOffsetAll()
  local plate
  for plate in pairs(self.plates or {}) do
    if plate then self:ApplyNameFontYOffset(plate) end
  end
end
