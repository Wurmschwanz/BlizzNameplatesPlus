...
local function SetYOffsetMouseMode(plate, wrapper, offset)
  ...
  local y = tonumber(offset) or 0
  local useWrapper = y > 0

  -- Vanilla nameplates are native Buttons. Moving only their child visuals
  -- leaves Blizzard's native mouse rectangle behind at the original world
  -- position. ShaguPlates solves the same problem by making the moved visual
  -- overlay itself clickable, disabling mouse input on the native parent, and
  -- forwarding clicks to parent:Click(). Do the same here.
  --
  -- Because BNPScaleWrapper is also the exact frame that is visually shifted,
  -- its clickable area cannot drift away from the visible plate, regardless of
  -- whether the configured offset is 5, 25 or 50 pixels.
  if plate.EnableMouse then
    plate:EnableMouse(not useWrapper)
  end

  if wrapper.EnableMouse then
    wrapper:EnableMouse(useWrapper)
  end

  if useWrapper then
    if wrapper.RegisterForClicks then
      wrapper:RegisterForClicks("LeftButtonUp")
    end
