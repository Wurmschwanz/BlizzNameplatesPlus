...
local function DarkComboPointBorderActive()
  -- Keep this independent from both ShaguTweaks and Dark Nameplate Border.
  -- Players can choose the dark Combo Point rim without changing any other UI.
  return BNP.IsDarkComboPointBorderEnabled
    and BNP:IsDarkComboPointBorderEnabled()
end

local function ApplyComboPointTexture(point, force)
  if not point or not point.texture then return end
  local dark = DarkComboPointBorderActive() and true or false

  -- Gold and dark are stored side by side in one atlas. Switching texture
  -- coordinates is reliable on old 1.12 clients and avoids their texture-file
  -- cache retaining the gold asset when a second filename is selected.
  if force or point.BNPComboPointTexture ~= CUSTOM_POINT_TEXTURE then
    point.texture:SetTexture(CUSTOM_POINT_TEXTURE)
    point.BNPComboPointTexture = CUSTOM_POINT_TEXTURE
  end

  if force or point.BNPComboPointDarkBorder ~= dark then
    if dark then
      point.texture:SetTexCoord(0.5, 1, 0, 1)
    else
      point.texture:SetTexCoord(0, 0.5, 0, 1)
    end
    if point.texture.SetVertexColor then
      point.texture:SetVertexColor(1, 1, 1, 1)
    end
    point.BNPComboPointDarkBorder = dark
  end
end

local function ApplyComboPointHolderStyle(holder, force)
  if not holder or not holder.points then return end
  local i
  for i = 1, MAX_POINTS do
    ApplyComboPointTexture(holder.points[i], force)
  end
end
...
local function SyncComboOwnerFromAPI(allowZeroOnOwner)
  ...
  if allowZeroOnOwner and comboOwnerGUID == targetGUID and not pendingGenerator then
    if comboOwnerCount > 0 then
      BNP.spentComboSnapshot = {
        guid = comboOwnerGUID,
        count = comboOwnerCount,
...
-- Finishers need the points that belonged to their exact cast target. Prefer
-- our GUID owner because the client may already report zero after spending the
-- points; fall back to the live API only for the matching current target.
function BNP:GetTrackedComboPointsForGUID(guid)
  if guid and comboOwnerGUID == guid and comboOwnerCount > 0 then
    return comboOwnerCount
  end

  local snapshot = BNP.spentComboSnapshot
  local now = GetTime and GetTime() or 0
  if guid and snapshot and snapshot.guid == guid
    and now - (snapshot.time or 0) <= 1.0
    and (snapshot.count or 0) > 0 then
    return snapshot.count
  end

  local targetGUID = GetTargetGUID()
  if targetGUID and (not guid or targetGUID == guid) then
...
local function CreateCustomPoint(parent)
  ...
  point.texture = tex
  point.BNPComboPointTexture = CUSTOM_POINT_TEXTURE
  point.BNPComboPointDarkBorder = false
  ApplyComboPointTexture(point)
...
local function EnsurePoints(plate)
  if plate.BNPComboPoints then
    ...
    ApplyComboPointHolderStyle(plate.BNPComboPoints)
  ...
  ApplyComboPointHolderStyle(holder)
...
-- Settings can be changed while no unit owns Combo Points. Update every
-- already-created holder directly instead of depending on UpdatePlate(), which
-- correctly skips plates without an active Combo Point owner.
function BNP:RefreshComboPointBorderStyle()
  local plate
  for plate in pairs(BNP.plates or {}) do
    if plate and plate.BNPComboPoints then
      ApplyComboPointHolderStyle(plate.BNPComboPoints, true)
    end
  end
end
